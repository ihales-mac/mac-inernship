...
ALTER PROCEDURE [dbo].[Login]
	@username nchar(50),
	@password char(50),
	@return_value nvarchar(10) OUTPUT 
AS
	...
	if (@FullName = null)
		BEGIN
			if((Select dbo.PrintName(FirstName,LastName) From [User] WHERE (Username=@username or Email=@username)) = null)
				BEGIN
					SELECT @return_value = 'INV USER'
				END
			else
				BEGIN
					SELECT @return_value = 'INV PW' 
				END
		END
	ELSE
		BEGIN
			SELECT @return_value = @FullName
		END
	INSERT INTO [Log] (Response,Username) VALUES (@return_value,@username)
return 123
GO
