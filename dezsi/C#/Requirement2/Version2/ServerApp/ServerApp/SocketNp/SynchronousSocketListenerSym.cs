...
namespace ServerApp.SocketNp
{
    ...
    public class SynchronousSocketListenerSym : ICommunication<Socket>
    {
        ...
        public void SendMessage(Socket handler, string rec, Header type) {
            switch (type)
            {
                ...
                case Header.Message:
                    ...
                    Send(handler, JMessage.Serialize(JMessage.FromValue<IList<CommonApp.Model.Message>>(ServerApp.Messages.GetMessagesOfUser(message.UserNameFrom), Header.Message)));
                    break;

                case Header.Messages:
                    string user = JMessage.Deserialize(rec).ToValue<string>();
                    Send(handler, JMessage.Serialize(JMessage.FromValue<IList<CommonApp.Model.Message>>(ServerApp.Messages.GetMessagesOfUser(user), Header.Messages)));
                    break;
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        SendMessage(handler, msg, jmessage.Type);
                      ...
                      //  Console.WriteLine("Truncated array {0}", Convert.ToBase64String(res));
                        ...
                        SendMessage(handler, decr, jmessage.Type);
